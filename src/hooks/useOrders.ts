...
import { loadOrders, saveOrders, loadActiveOrderId, saveActiveOrderId } from "#/lib/order-storage";
...
export default function useOrders() {
  ...
  function renameOrder(id: string, name: string) {
    ...
    setOrders((prev) => prev.map((o) => (o.id === id ? { ...o, name: safeName } : o)));
  }
    ...
    setOrders((prev) =>
      prev.map((o) =>
        o.id === activeOrderId
          ? {
              ...o,
              ingredients: o.ingredients.map((ing) => (ing.id === id ? { ...ing, ...patch } : ing)),
            }
          : o,
      ),
    );
